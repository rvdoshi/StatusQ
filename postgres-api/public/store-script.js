...
async function fetchAndPopulateTable(selectedCategory) {
   ...
   // console.log(selectedCategory);
    try {
        const apiUrl = selectedCategory ? `http://103.113.142.236:3002/api/filter/stores_data?category=${selectedCategory}` : 'http://103.113.142.236:3002/api/filter/stores_data';
      //   console.log(apiUrl);
      ...
      //   console.log(response);
        ...
        if (response.ok) {
            ...
            data.rows.forEach((row, index) => {
                ...
                <td>${formatDate(row.stock_check || '')}</td>
                <td>${formatDate(row.m_i_p||'')}</td>
                <td>${formatDate(row.packing || '')}</td>
