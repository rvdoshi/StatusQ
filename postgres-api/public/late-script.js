...
async function fetchAndPopulateTable(selectedCategory) {
  ...
  try {
    ...
    // const apiUrl = `http://103.113.142.236:3002/api/filter/team_data?category=Stock`;
    ...
    if (response.ok) {
      ...
      data.rows.forEach((row) => {
        // Skip rows where row.complete is true or row.accounts_invoicing is not null
        if (row.complete === true || row.accounts_invoicing !== null) {
          return;
        }

        // Check if the delivery_date has passed the current date
        const deliveryDate = new Date(row.delivery_date);
        if (deliveryDate > currentDate) {
          return; // Skip if delivery_date is in the future
        }
        ...
        if (deliveryDate < currentDate) {
          pastDueRowCount++;
          tableRow.classList.add("past-due-row");
        }
