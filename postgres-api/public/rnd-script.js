...
//    console.log(selectedCategory);
    selectedCategory = "New"; // hardcoded need to change
    try {
        const apiUrl = selectedCategory ? `http://103.113.142.236:3002/api/filter/rnd_data?category=${selectedCategory}` : 'http://103.113.142.236:3002/api/filter/rnd_data';
      //   console.log(apiUrl);
      ...
      //   console.log(response);
        ...
        if (response.ok) {
            ...
            data.rows.forEach((row, index) => {
                ...
                <td>${formatDate(row.specification_check||'')}</td>
                <td>${formatDate(row.bom || '')}</td>
                <td>${formatDate(row.rnd_complete || '')}</td>
                <td>${(row.notes || '')}</td>
        `;
