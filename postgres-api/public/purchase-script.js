...
async function fetchAndPopulateTable(selectedCategory) {
    ...
    try {
        const apiUrl = selectedCategory ? `http://103.113.142.236:3002/api/filter/purchase_data?category=${selectedCategory}` :
            'http://103.113.142.236:3002/api/purchase_data';
        //   console.log(apiUrl);
        ...
        if (response.ok) {
            ...
            data.rows.forEach((row, index) => {
                ...
                <td>${formatDate(row.material_status || '')}</td>
                <td>${formatDate(row.mrp||'')}</td>
                <td>${formatDate(row.po || '')}</td>
                <td>${formatDate(row.material_receipt_eta || '')}</td>
                <td>${(row.market_shortage || '')}</td>
                <td>${(row.price_hike || '')}</td>
                <td>${(row.notes || '')}</td>
        `;
