...
async function fetchAndPopulateTable(selectedCategory) {
   ...
   // console.log(selectedCategory);
    try {
        const apiUrl = selectedCategory ? `http://103.113.142.236:3002/api/filter/sales_data?category=${selectedCategory}` : 'http://103.113.142.236:3002/api/filter/sales_data';
      //   console.log(apiUrl);
      ...
      //   console.log(response);
        ...
        if (response.ok) {
            ...
            data.rows.forEach((row, index) => {
                ...
                <td>${formatDate(row.late_clause || '')}</td>
                <td>${formatDate(row.so_check_with_quotation||'')}</td>
                <td>${formatDate(row.specs_check || '')}</td>
