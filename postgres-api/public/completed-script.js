...
async function fetchAndPopulateTable(selectedCategory) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            let populatedRowCount = 0; // Counter for populated rows
            pastDueRowCount = 0;

            data.rows.forEach((row) => {
                // Only populate rows where accounts_invoicing is not null
                if (row.accounts_invoicing === null && (row.complete === null || row.complete === false)) {
                    return;
                }
              ...
              `;
                ...
                // Increment the populated rows counter
                populatedRowCount++;
            });

            // Log the number of rows populated
            console.log(`Number of rows populated in the table: ${populatedRowCount}`);
